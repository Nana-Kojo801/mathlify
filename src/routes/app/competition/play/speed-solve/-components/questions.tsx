...
import { generateSpeedSolveMarathonQuestions } from '@/lib/helpers'
import { useActions, useDifficulty, useScore } from '@/components/game-modes/speed-solve/speed-solve-game-store'

const Questions = ({ onResult }: { onResult: (totalTime: number) => void }) => {
  const [selected, setSelected] = useState<number | null>(null)
  const score = useScore()
  const baseDifficulty = useDifficulty()!
  const [difficulty, setDifficulty] = useState(generateSpeedSolveMarathonQuestions(baseDifficulty, score))
  ...
  const handleOptionClicked = (answer: number, index: number) => {
    ...
    if (answer === difficulty.correctAnswer) {
      ...
      const newDifficulty = generateSpeedSolveMarathonQuestions(baseDifficulty, score + 1)
